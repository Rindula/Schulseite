...
while ($ar = $result->fetch_assoc()) {
    ...
    $out .= "<td class='datum'>".strftime("%A", $ar["Datum"]).", $day.$month.$year ($days)</td>";
    $text = "whatsapp://send?text=".whatsNewLine("*Hausaufgabe*\nFach: _".$ar["fach"]."_\nZu erledigen bis: *".strftime("%A", $ar["Datum"]).", $day.$month.$year*" . ((!empty($tasks)) ? "\n\nAufgabe(n):$tasks" : ""));
    ...
    if (is_dir($IMAGEPATH) == 1 && !is_dir_empty($IMAGEPATH)) {
        ...
        foreach (glob($IMAGEPATH . '*') as $filename) {
            if (is_image($filename)) {
                $lnk++;
                $out .= "<a data-title='".$ar["fach"]." Hausaufgabe bis zum ".strftime("%A", $ar["Datum"]).", $day.$month.$year' data-lightbox='loesungen-" . $ar["ID"] . "' href='loesungen/" . $ar["ID"] . "/" . basename($filename) . "'>Lösung Seite " . $lnk . "</a><br>";
            }
        }
    }
